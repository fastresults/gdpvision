...
function interpretation(kind: MapFeature["kind"], title: string): Pick<MapFeature, "signal" | "trend" | "impact" | "forecast"> {
  ...
  if (kind === "place") return { signal: "A nation outline in this Caribbean projection (small islands enlarged for visibility, not to scale).", trend: "Not applicable to a location marker.", impact: "It provides orientation and identifies the selected country or a regional comparator.", forecast: "Not applicable to a location marker." };
...
export function RegionMap({ code, countryName, layers, flows = [], flowPartners = [], onResearchPartners, researchingPartners = false, kpis = [], sectors = [], evidence = { sources: [], memory: [] }, live, focusedLayerId, pinnedFeature, onPin, onEvidence, initialMode = "regional" }: {
  ...
  const legend = useMemo(() => [
    { show: true, symbol: "selected", label: "Selected country", signal: "The gold island outline identifies the country currently being analysed; other nations are drawn as paper outlines. Small islands are enlarged for visibility and are not to scale.", impact: "All country-anchored indicators and flows are read in relation to this location." },
    ...
    <section className="relative overflow-hidden border border-ink-950 bg-paper-0">
      ...
      <div ref={mapRef} className="relative min-h-[620px] overflow-hidden bg-paper-50">
          ...
          <svg ref={svgRef} style={mapMode === "globe" ? { display: "none" } : undefined} viewBox={`${view.x} ${view.y} ${view.s * 100} ${view.s * 100}`} role="img" aria-label={`${countryName} sovereign intelligence map`} className={`absolute inset-0 h-full w-full touch-none ${panning ? "cursor-grabbing" : "cursor-grab"}`} onPointerDown={beginPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
            ...
            {regional && POINTS.map((point) => { const p = project(point.lon, point.lat); const isSelected = point.code === code.toUpperCase(); const shape = shapePath(point.code, (lon, lat) => { const q = project(lon, lat); return [q.x, q.y]; }, (isSelected ? 3.2 : 2) * view.s); const feature = mapFeature({ id: `place-${point.code}`, kind: "place", title: point.name, value: isSelected ? "Selected country" : "Regional comparator", meta: `${point.lat.toFixed(2)}°, ${point.lon.toFixed(2)}°${shape?.enlarged ? " · outline enlarged for visibility, not to scale" : ""}` }); const lx = (shape ? shape.cx + shape.halfW : p.x) + 1; return <g key={point.code} {...interaction(feature)}><circle cx={shape?.cx ?? p.x} cy={shape?.cy ?? p.y} r={3.8 * Math.min(1, view.s * 1.5)} className="fill-transparent" />{shape ? <path d={shape.d} className={isSelected ? "fill-gold-500 stroke-ink-950" : "fill-paper-0 stroke-ink-500"} strokeWidth={(isSelected ? 0.35 : 0.2) * view.s} strokeLinejoin="round" /> : <circle cx={p.x} cy={p.y} r={isSelected ? 2.3 : 0.9} className={isSelected ? "fill-gold-500 stroke-ink-950" : "fill-paper-0 stroke-ink-500"} strokeWidth={isSelected ? 0.55 : 0.3} />}<text x={lx} y={(shape?.cy ?? p.y) - 1.2} className={isSelected ? "fill-ink-950 font-mono text-[2px]" : "fill-ink-600 font-mono text-[1.7px]"}>{isSelected ? point.name : point.code}</text></g>; })}
