...
export const Route = createFileRoute("/_authenticated/admin/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Instance administration: users, roles, and country bindings.",
      },
    ],
  }),
    ...
    <div className="min-h-dvh bg-paper-0 text-ink-950">
      <header className="flex items-center justify-between border-b border-line-200 px-8 py-5">
        ...
        <div className="flex items-center gap-6 font-mono text-[11px] uppercase tracking-[0.2em] text-ink-500">
          <Link to="/admin/countries" className="hover:text-ink-950">
            Country onboarding
          </Link>
          <Link to="/config" className="hover:text-ink-950">
            Configuration
          </Link>
          <Link to="/admin/audits/keying" className="hover:text-ink-950">
            Keying audit
          </Link>
          <Link to="/admin/audits/log" className="hover:text-ink-950">
            Audit log
          </Link>
      ...
      <main className="mx-auto max-w-7xl px-8 py-16">
        ...
        <p className="mt-3 max-w-2xl text-sm text-ink-500">
          Grant portfolio roles, wire ministers to their country instance, and confirm each
          principal's default nation.
        </p>
        ...
        <section className="mt-10 border border-line-200 p-6">
          ...
          <p className="mt-2 text-sm text-ink-500">
            Open a country's console to review access requests, assign country-scoped roles, and
            seed GDP, sectors, and ministries.
          </p>
        ...
        <div className="mt-12 space-y-8">
          {users.map((u) => (
            <UserCard
              ...
              countries={countries}
              onGrant={(role, cc) =>
                mut.mutate(() => grant({ data: { userId: u.user_id, role, countryCode: cc } }))
              }
              onRevoke={(role, cc) =>
                mut.mutate(() => revoke({ data: { userId: u.user_id, role, countryCode: cc } }))
              }
              onBind={(cc) =>
                mut.mutate(() => bind({ data: { userId: u.user_id, countryCode: cc } }))
              }
              onUnbind={(cc) =>
                mut.mutate(() => unbind({ data: { userId: u.user_id, countryCode: cc } }))
              }
              onDefault={(cc) =>
                mut.mutate(() => setDefault({ data: { userId: u.user_id, countryCode: cc } }))
              }
            />
          ))}
          {users.length === 0 && (
            <p className="border border-line-200 p-12 text-center text-sm text-ink-500">
              No users yet. Use the invite form below to bring your first principal onto the
              instance.
            </p>
          )}
        </div>
  ...
  return (
    <section className="mt-20 border-t border-line-200 pt-10">
      ...
      <p className="mt-2 max-w-2xl text-sm text-ink-500">
        Sends a Lovable Cloud email invitation. Assign roles and country bindings after they accept.
      </p>
      <div className="mt-6 flex flex-wrap items-end gap-3 text-sm">
        <label className="flex flex-col gap-1">
          ...
          <input
            ...
            className="w-72 border border-line-200 px-2 py-1"
          />
        </label>
        <label className="flex flex-col gap-1">
          <span className="font-mono text-[10px] uppercase tracking-widest text-ink-500">
          ...
          <input
            ...
            className="w-56 border border-line-200 px-2 py-1"
          />
        </label>
        <button
          type="button"
...
function ConfigSection({
  ...
  return (
    <section className="mt-20 border-t border-line-200 pt-10">
      ...
      <p className="mt-2 max-w-2xl text-sm text-ink-500">
        Edit JSON values directly. Changes are audited.
      </p>
      {error && <p className="mt-3 text-sm text-red-600">{error.message}</p>}
      <ul className="mt-8 space-y-6">
        {rows.length === 0 && (
          <li className="text-sm text-ink-500">No configuration keys defined yet.</li>
        )}
        {rows.map((r) => (
...
function ConfigRow({
  ...
  row: { key: string; value_json: any; updated_at: string };
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        ...
        <div>
                ...
                <span className="flex gap-3 font-mono text-[10px] uppercase tracking-widest">
                  {!b.is_default && (
                    <button
                      ...
                      onClick={() => onDefault(b.country_code)}
                    >
                      Set default
                    </button>
                  )}
                  <button
                    ...
                    onClick={() => onUnbind(b.country_code)}
                  >
                    Remove
                  </button>
                </span>
          ...
          <div className="mt-4 flex gap-2 text-sm">
            <select
              value={bindCc}
              onChange={(e) => setBindCc(e.target.value)}
              className="border border-line-200 px-2 py-1"
            >
              {countries.map((c) => (
