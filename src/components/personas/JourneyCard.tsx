...
type Props = {
  ...
  /**
   * When true, prerequisites are not yet met. The card is still navigable —
   * only the visual status and CTA copy change to coach the user.
   */
  disabled?: boolean;
  disabledHint?: string;
  currentHere?: boolean;
...
export function JourneyCard({
  ...
  const status = currentHere ? "here" : count > 0 ? "ready" : "empty";
  const statusColor =
    status === "ready"
      ? "bg-emerald-500"
      : status === "here"
  ...
  const effectiveCta = disabled ? disabledHint ?? cta : cta;
      ...
      <div className="mt-auto pt-4">
        {currentHere ? (
          ...
          <span className="inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-[0.18em] text-ink-950 group-hover:gap-2 transition-all">
            {effectiveCta} <ArrowRight size={12} />
          </span>
        )}
      </div>
    </div>
