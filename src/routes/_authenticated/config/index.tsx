...
export const Route = createFileRoute("/_authenticated/config/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Provision instances: browse the CARICOM/OECS registry, review Country Packs, and generate National Signatures." },
    ],
  }),
  ...
  return (
    <div className="min-h-screen bg-paper-0 text-ink-950">
      ...
      <main className="mx-auto grid max-w-7xl grid-cols-[minmax(0,1fr)_minmax(0,420px)] gap-12 px-8 py-16">
        <section>
          <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-ink-500">Instance provisioning · Screen 0</p>
          <h1 className="mt-2 font-serif text-4xl">The registry</h1>
          <p className="mt-3 max-w-2xl text-sm text-ink-500">
            Twenty-two CARICOM &amp; OECS member states. Select a row to review the Country Pack and activate the
                        ...
                        <li key={r.code}>
                          ...
                          >
                            ...
                            <span>
                              {r.name}
                              {r.cbiState && (
                                <span className="ml-2 font-mono text-[10px] uppercase tracking-widest text-ink-500">CBI</span>
                              )}
                            </span>
        ...
        <aside className="sticky top-8 h-fit border-l border-line-200 pl-8">
          {selected ? (
            ...
            <div className="text-sm text-ink-500">Select a nation to review the Country Pack, activate the instance,
              confirm the portfolio→sector map, and generate the National Signature.</div>
          )}
        </aside>
      </main>
    </div>
...
function PackPanel({ code }: { code: string }) {
  ...
  const activateMut = useMutation({
    mutationFn: (v: { makeDefault: boolean }) => activate({ data: { code, makeDefault: v.makeDefault } }),
    ...
    <div className="space-y-8 text-sm">
      <header>
        <p className="font-mono text-[10px] uppercase tracking-widest text-ink-500">
          {data.code} · {data.tier}{data.cbiState ? " · CBI" : ""}
        </p>
      ...
      <section>
        <h3 className="font-mono text-[10px] uppercase tracking-widest text-ink-500">Portfolio → sector map</h3>
        <ul className="mt-3 divide-y divide-line-200 border-y border-line-200">
      ...
      <section>
        <h3 className="font-mono text-[10px] uppercase tracking-widest text-ink-500">Sector shares</h3>
      ...
      <SignaturePanel preview={data} pending={genSigMut.isPending} onGenerate={() => genSigMut.mutate()} error={genSigMut.error as Error | null} />
    </div>
  );
}
  ...
  return (
    <section>
      <div className="flex items-baseline justify-between">
        <h3 className="font-mono text-[10px] uppercase tracking-widest text-ink-500">National Signature</h3>
        <button
          onClick={onGenerate}
          disabled={pending}
      ...
      {!sig ? (
        ...
        <div className="mt-3 space-y-3">
          ...
          <div>
            <h4 className="font-mono text-[10px] uppercase tracking-widest text-ink-500">Pillars</h4>
            <ul className="mt-1 space-y-1 text-[12px]">
              {sig.pillars.map((p) => (
                <li key={p.name}><span className="font-medium">{p.name}.</span> {p.thesis}</li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="font-mono text-[10px] uppercase tracking-widest text-ink-500">Distinctives</h4>
            <ul className="mt-1 list-disc pl-4 text-[12px]">
              {sig.distinctives.map((d, i) => <li key={i}>{d}</li>)}
            </ul>
          </div>
          <div>
            <h4 className="font-mono text-[10px] uppercase tracking-widest text-ink-500">Risks</h4>
            <ul className="mt-1 list-disc pl-4 text-[12px]">
              {sig.risks.map((d, i) => <li key={i}>{d}</li>)}
            </ul>
          </div>
