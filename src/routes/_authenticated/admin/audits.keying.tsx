...
const auditsQuery = queryOptions({
  queryKey: ["keying-audits"],
  queryFn: () => listKeyingAudits(),
});
...
export const Route = createFileRoute("/_authenticated/admin/audits/keying")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Universal country + sector keying audit across all domain tables.",
      },
    ],
  }),
    ...
    <div className="min-h-dvh bg-paper-0 text-ink-950">
      ...
      <main className="mx-auto max-w-6xl px-8 py-16">
        ...
        <p className="mt-3 max-w-2xl text-sm text-ink-500">
          Walks every domain table and confirms each row carries valid country_code and (where
          applicable) sector_code references. A clean audit is a v1.0 release gate.
        </p>

        {mut.isError && <p className="mt-6 text-sm text-red-600">{(mut.error as Error).message}</p>}
          ...
          <section className="mt-12">
            <header className="flex items-baseline justify-between border-b border-line-200 pb-3">
              ...
              <span className="font-mono text-[11px] text-ink-500">
                {new Date(latest.ran_at).toLocaleString()} · {latest.total_checked.toLocaleString()}{" "}
                rows ·{" "}
                <span
                  className={latest.total_violations === 0 ? "text-emerald-700" : "text-red-600"}
                >
          ...
          <section className="mt-16">
            ...
            <ul className="mt-4 divide-y divide-line-200 border-y border-line-200">
              {audits.slice(1).map((a) => (
                <li
                  key={a.id}
                  className="grid grid-cols-[1fr_auto_auto] gap-6 py-3 font-mono text-[11px] text-ink-500"
                >
