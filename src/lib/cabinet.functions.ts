  ...
  .handler(async ({ data, context }): Promise<RoomOverview> => {
    ...
    const [{ data: sessions }, { data: commits }, { data: decisions }, { data: signals }, { data: strategies }, { data: scenarios }, { data: alerts }] =
      await Promise.all([
        ...
        supa.from("intake_items").select("id,topic,priority,scope_key,created_at").eq("scope_key", cc).in("priority", ["P1","P2"]).is("story_key", null).order("created_at",{ascending:false}).limit(8),
        ...
        supa.from("scenarios").select("id,name,status,updated_at").eq("country_code", cc).order("updated_at",{ascending:false}).limit(8),
