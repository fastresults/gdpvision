          ...
          return (
            <li key={s.key}>
              <Link
                ...
                title={s.locked ? (!briefCommitted ? "Commit the brief to unlock" : "Approve the programme plan to unlock") : undefined}
                onClick={(e) => { if (s.locked) e.preventDefault(); }}
                className={cn(
                  ...
                  isActive ? "border-ink-950" : s.complete ? "border-emerald-500/60 hover:border-ink-950" : "border-line-200 hover:border-ink-500",
                )}
              >
                <span
                  className={cn(
                ...
                >
                  {s.locked ? <Lock size={11} /> : s.complete && !isActive ? <Check size={12} strokeWidth={3} /> : s.n.toString().padStart(2, "0")}
                </span>
                <span className="min-w-0">
                  <span className={cn("flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.2em]", isActive ? "text-ink-950" : "text-ink-500")}>
                    <Icon size={11} /> {s.sub}
                  </span>
                  <span className="mt-0.5 block font-serif text-[15px] leading-tight text-ink-950">{s.label}</span>
                  <span className="mt-0.5 block font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500">{s.hint}</span>
                </span>
              </Link>
            </li>
          );
