...
export const Route = createFileRoute("/_authenticated/admin/documents")({
  head: () => ({
    meta: [{ title: "Documents — GDPVision" }, { name: "robots", content: "noindex" }],
  }),
    ...
    <div className="min-h-dvh bg-paper-0 text-ink-950">
      ...
      <main className="mx-auto max-w-6xl px-8 py-16">
        ...
        <p className="mt-3 max-w-2xl text-sm text-ink-500">
          Rendered artifacts from Cabinet decisions, Briefing packs, FDI packages, Term reports, and
          State-of-the-Mandate briefings. Preview inline; use the browser's print dialog to save as
          PDF.
        </p>
        ...
        {rows.length === 0 ? (
          ...
          <ul className="mt-10 divide-y divide-line-200 border-y border-line-200">
            {rows.map((r) => (
              <li
                key={r.id}
                className="grid grid-cols-[auto_1fr_auto_auto] items-baseline gap-6 py-4 text-sm"
              >
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-paper-0/95 p-6">
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="flex gap-4 font-mono text-[10px] uppercase tracking-widest">
              ...
              <button onClick={() => setPreview(null)} className="hover:text-red-600">
                Close
              </button>
            </div>
          </div>
          <iframe
            title={preview.title}
            srcDoc={preview.html}
            className="flex-1 border border-line-200 bg-white"
          />
        </div>
      )}
    </div>
