// The PRD as a document, for reading and printing. No chamber chrome: a
...
import { getPrd } from "@/lib/egov/prd.functions";
...
export const Route = createFileRoute(
  ...
  head: ({ params }) => ({
    meta: [
      { title: `PRD document · ${params.code} — GDPVision` },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: PrdDocumentPage,
});

function PrdDocumentPage() {
  const { code, prdId } = Route.useParams();
  const fetchPrd = useServerFn(getPrd);
  const q = useQuery({
    queryKey: ["egov-prd", code, prdId],
    queryFn: () => fetchPrd({ data: { code, prdId } }),
  });
  const d = q.data;

  useEffect(() => {
    if (!d) return;
    document.title = `${d.prd.title} — PRD v${d.prd.version}`;
  }, [d]);
        ...
        <div className="mx-auto flex max-w-[1280px] flex-wrap items-center gap-x-5 gap-y-2 px-4 py-2.5 sm:px-6">
          ...
          {d ? (
            <div className="flex min-w-0 flex-wrap items-baseline gap-x-3 text-[13px]">
              <span className="truncate text-ink-950">{d.prd.title}</span>
              <span className="text-ink-500">v{d.prd.version}</span>
              <span
                className={cn(
                  "font-mono text-[10px] uppercase tracking-[0.16em]",
                  PRD_META[d.prd.status].text,
                )}
              >
                {PRD_META[d.prd.status].label}
              </span>
            </div>
          ) : null}
          <button
      ...
      <main className="px-4 py-8 sm:px-6 print:p-0">
        ...
        {d ? (
          <PrdDocument
            ...
            version={d.prd.version}
            status={PRD_META[d.prd.status].label}
            approvedAt={d.prd.approved_at}
            platformName={d.prd.scope.platform_name}
            accent={d.brand.accent}
            border={d.brand.borders[0] ?? d.brand.ink}
            sections={d.sections}
          />
        ) : null}
      </main>
