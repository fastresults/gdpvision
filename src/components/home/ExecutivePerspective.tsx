...
type PerspectiveContextValue = {
  active: ExecutivePerspective | null;
  pinned: boolean;
  schedule: (perspective: ExecutivePerspective, delay: number) => void;
  clear: () => void;
  show: (perspective: ExecutivePerspective) => void;
  togglePin: (perspective: ExecutivePerspective) => void;
  close: () => void;
};
...
export function ExecutivePerspectiveProvider({ children }: { children: ReactNode }) {
  const [active, setActive] = useState<ExecutivePerspective | null>(null);
  ...
  const schedule = useCallback(
    (perspective: ExecutivePerspective, delay: number) => {
      cancelTimer();
      if (pinned) return;
      timer.current = setTimeout(() => setActive(perspective), delay);
    },
    [cancelTimer, pinned],
  );
...
export function useExecutivePerspective(perspective: ExecutivePerspective) {
  const context = useContext(PerspectiveContext);
  if (!context) throw new Error("useExecutivePerspective must be used inside its provider");
  const active = context.active?.id === perspective.id;

  return {
    tabIndex: 0,
    "data-perspective-active": active ? "true" : undefined,
    onMouseEnter: () => context.schedule(perspective, HOVER_DELAY_MS),
