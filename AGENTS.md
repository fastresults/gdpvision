...
- **Illustrations**: one house style everywhere (UI + print) — monochrome hand-drawn engraved graphite sketch, rendered through `<Illustration>` from `@/components/marketing/Illustration`. Never a bare `<img>` for an illustration, never colour. Sole exception: the marketing hero. Spec + prompt template: `docs/illustration-contract.md`. Assets: `src/assets/illustrations/*.asset.json`, `src/assets/chambers/*.asset.json`.
