...
- **Explain this**: any derived figure, coefficient, assumption or model-authored output a reader could question must be interrogable — wrap it in `<Explain id="...">` from `@/components/explain/Explain` and register the rationale in `src/lib/explain/<domain>-entries.ts` (calling `registerRationales`). Never explain a number with an inline `title` attribute or a bare tooltip. Surfaces that already open a full detail modal (executive brief `DetailModal`) satisfy the rule and must not double up.
